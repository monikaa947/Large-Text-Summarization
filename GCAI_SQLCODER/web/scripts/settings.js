...
function initSdk(name) {
    ...
    const chatWidgetSettings = {
        URI: 'idcs-oda-3038f35fa1204cf688fdfab3640fc5ed-da4.data.digitalassistant.oci.oraclecloud.com',                               // ODA URI, only the hostname part should be passed, without the https://
        ...
        channelId: '7dccb4e8-b5a1-427c-99b3-b80fc8d0aee0',                   // Channel ID, available in channel settings in ODA UI, optional if client auth enabled
        ...
        i18n: {                                     // Provide translations for the strings used in the widget
            en: {                                   // en locale, can be configured for any locale
                chatTitle: 'FiNMATE'       // Set title at chat header
            }
        },
        ...
        icons: {
            logo: "https://mastekgroup.sharepoint.com/_catalogs/masterpage/ChatBot/images/BotButton.png",
            launch: "https://mastekgroup.sharepoint.com/_catalogs/masterpage/ChatBot/images/BotButton.png",
            ...
            avatarBot:  './style/AI_AMIGO.png' //'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36" fill="none"><path d="M0 0h36v36H0V0z" fill="#C74634"/><path fill-rule="evenodd" clip-rule="evenodd" d="M7.875 8.625a2.25 2.25 0 00-2.25 2.25v16c0 .621.504 1.125 1.125 1.125h.284c.298 0 .585-.119.796-.33l2.761-2.76a2.25 2.25 0 011.59-.66h15.944a2.25 2.25 0 002.25-2.25V10.875a2.25 2.25 0 00-2.25-2.25H7.875zM24.75 18a2.25 2.25 0 100-4.5 2.25 2.25 0 000 4.5zm-4.5-2.25a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-9 2.25a2.25 2.25 0 100-4.5 2.25 2.25 0 000 4.5z" fill="#fff"/></svg>'
        },
        width: "1026px",
        height: "1026px"
    };
    ...
    if (enableCustomCards) {
        chatWidgetSettings.delegate = {
            render: (message) => {
                if (message.messagePayload.type === "card" && message.messagePayload.cards[0].channelExtensions) {
                    console.log(message);
                    const chart = JSON.parse(message.messagePayload.cards[0].channelExtensions.chart)
                    ...
                    cards.forEach(card => {
                        ...
                        Plotly.newPlot(chartElem, chart.data, chart.layout)
                        titleElem.appendChild(chartElem)
                            ...
                            actions.forEach(action => {
                                ...
                                if (action.type == "url") {
                                    button.addEventListener('click', () => {
                                        actionHandler(action.url, card.channelExtensions.chart_data);
                                    });
                                }
                                if (action.type == "postback") {
                                    button.addEventListener('click', () => {
                                        postbackActionHandler(action);
                                    });
                                }
                        ...
                        cardElem.appendChild(actionsElem)
                    });
    ...
    function actionHandler(action, data) {
        ...
        fetch(action,
            ...
            .then(blob => {
                ...
                // Specify the file name (if known)
                link.setAttribute('download', `data_${new Date().toISOString()}.csv`); // Set desired file name here
    ...
    function postbackActionHandler(action) {
        Bots.sendMessage(action);
    }
