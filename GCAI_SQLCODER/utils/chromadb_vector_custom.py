...
class ChromaDB_VectorStore(VannaBase):
    ...
    def _extract_documents(query_results) -> list:
        ...
        if "documents" in query_results:
            ...
            if len(documents) == 1 and isinstance(documents[0], list):
                ...
                if len(documents) > 1 and isinstance(documents, list):
                    try:
    ...
    def get_similar_question_sql(self, question: str, **kwargs) -> list:
        if not question:
        # If the question is empty, retrieve all documents
            return ChromaDB_VectorStore._extract_documents(
                self.sql_collection.get(
                    limit=5
                )
            )
        else:
            # Below code is added as an experimental feature
            question_sql_json = json.dumps(
                {
                    "question": question,
                },
            ...
            return ChromaDB_VectorStore._extract_documents(
                self.sql_collection.query(
                    query_texts=[question_sql_json],
                    n_results=self.n_results,
                    where_document={"$contains":"question"} #Added as Experimental feature
                )
            )
